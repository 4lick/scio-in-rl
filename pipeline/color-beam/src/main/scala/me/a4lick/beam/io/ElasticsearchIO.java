  ...
  public static class BoundedElasticsearchSource extends BoundedSource<String> {
    ...
    public List<? extends BoundedSource<String>> split(
      ...
      if (backendVersion == 2){
        ...
        // check aliases
        String index = getAlias(connectionConfiguration);
...
//    }
    ...
    private static String getAlias(ConnectionConfiguration connectionConfiguration) throws IOException {
      String index = connectionConfiguration.getIndex();
      HashMap<String, String> params = new HashMap<>();

      String endpoint = "/_cat/aliases?format=json";
      try (RestClient restClient = connectionConfiguration.createClient()) {
        ArrayNode response = (ArrayNode)parseResponse(
                restClient.performRequest("GET", endpoint));

        for (JsonNode alias : response) {
          if(alias.get("alias").asText().equals(connectionConfiguration.getIndex())) {
            index = alias.get("index").asText();
          }
        }
